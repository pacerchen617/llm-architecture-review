...
|----------|------|---------|--------|------|
| Instinct Engine | `instinct-engine.py` v3.1 | `call_ollama()` | 分類觀察類型 + 生成繁中摘要 | daily 03:00 + 每次 compact |
| Quality Gate | `_lib/models.py` | `quality_gate()` | 新 instinct/lesson 品質檢查（1-5 分） | 每次知識寫入前 |
| Cognize | `learning-log-utils.py` v8 | `quality_gate()` | 灰色地帶（noise 0.3-0.6）二次評分 | ~5 次/日 |
| Curiosity Loop | `curiosity-loop.py` | `call_ollama()` | 外部來源品質評估 | daily 04:30 |
| Evolution Loop | `evolution-loop.py` | `call_ollama()` | 活動評分 + 假說生成 | weekly |
| Vault Search | Obsidian plugin | `qwen3-embedding` | 筆記語意 embedding | 用戶搜尋時 |
...
from _lib.models import call_ollama, call_ollama_chat, quality_gate
```
...
# score ≥ 4 = pass, 3 = retry, ≤ 2 = escalate
```
